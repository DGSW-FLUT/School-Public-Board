...
function FormatTimeDifferance(diff_milliseconds) {
    ...
    if (result.hour !== null) result_string += `${result.hour}시간 `;
...
class PercentageGauage extends Component {
    render() {
        const fulltime = this.props.eject_date - this.props.inject_date;
        const lefttime = this.props.eject_date - this.props.nowDate;
        const nowtime = this.props.nowDate - this.props.inject_date;
        return (
            <>
                <S.Content>
                    {nowtime < fulltime ? `집까지 ${FormatTimeDifferance(lefttime)} 남았습니다.` : '집 갈 시간입니다!'}
                </S.Content>
                <S.Bar>
                    <S.OutterBar>
